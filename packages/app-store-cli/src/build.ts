...
`;
  ...
  filesToGenerate.forEach(([fileName, output]) => {
    const objectName = fileName.includes("calendar.services")
      ? "CalendarServiceMap"
      : fileName.includes("video.adapters")
      ? "VideoAdapterMap"
      : fileName.includes("payment.services")
      ? "PaymentServiceMap"
      : "";
    const rawOutput = `${banner}${output.join("\n")}`;
    const finalOutput = objectName ? addE2EGuard(rawOutput, objectName) : rawOutput;
    fs.writeFileSync(`${APP_STORE_PATH}/${fileName}`, formatOutput(finalOutput));
  });
...
function isVideoApp(app: App) {
  return !!app.categories?.includes("conferencing");
}
